...
int main(int argc, char *argv[])
{
    ...
    m_nan_value *input_array_for_m = malloc(num_inputs * sizeof(m_nan_value));
    int i;
    int num_outputs = m_num_inputs();
    m_nan_value *outputs_array_for_m = malloc(num_outputs * sizeof(m_nan_value));
    ...
    if (d)
    {
        while ((dir = readdir(d)) != NULL)
        {
            ...
            {
                input_array_for_m[i] = m_nan_undefined;
            }
            for (i = 0; i < num_outputs; i++)
            {
                outputs_array_for_m[i] = m_nan_undefined;
            }
            state = 0;
            ...
            while (EOF != fscanf(fp, "%[^\n]\n", line_buffer))
            {
                ...
                case 1:
                    ...
                    input_array_for_m[name_index] = m_nan_literal(value);
                    break;
