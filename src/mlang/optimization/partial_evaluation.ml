...
let rec partial_evaluation (ctx: ctx) (p: program) (e: expression Pos.marked) : expression Pos.marked =
  ...
  | Binop (op, e1, e2) ->
    let new_e1 = partial_evaluation ctx p e1 in
    let new_e2 = partial_evaluation ctx p e2 in
    Pos.same_pos_as begin match (Pos.unmark op, Pos.unmark new_e1, Pos.unmark new_e2) with
  ...
  | Unop (op, e1) ->
    let new_e1 = partial_evaluation ctx p e1 in
    Pos.same_pos_as begin match (Pos.unmark new_e1) with
      | Literal _ ->
  ...
  | Conditional (e1, e2, e3) ->
    ...
    begin match Pos.unmark new_e1 with
  ...
  | Index (var, e1) ->
    let new_e1 = partial_evaluation ctx p e1 in
    if VariableMap.mem (Pos.unmark var) ctx.ctx_current_scc then Pos.same_pos_as (Literal Undefined) e
    else
      begin match Pos.unmark new_e1 with
        | Literal Undefined -> Pos.same_pos_as (Literal Undefined) e
        | Literal l ->
          ...
          in
          begin match (VariableMap.find (Pos.unmark var) p.program_vars).var_definition with
            ...
            | TableVar (size, IndexGeneric e') ->
              ...
              else begin match Pos.unmark e' with
                | Literal _ | Var _ -> e'
  ...
  | LocalVar lvar -> begin try Pos.same_pos_as (
    ...
    begin match Pos.unmark new_e1 with
      | Literal _ | Var _ ->
        let new_ctx =
          { ctx with
            ctx_local_vars =
              LocalVariableMap.add lvar (expr_to_partial (Pos.unmark new_e1))
                ctx.ctx_local_vars
          }
        in
  ...
  | FunctionCall (((ArrFunc | InfFunc | PresentFunc | NullFunc) as f), [arg]) ->
    ...
    begin match Pos.unmark new_arg with
      | Literal _ ->
        Pos.same_pos_as
          (Mvg.Literal
          ...
          | Not_found ->
            try
              ...
              | (Literal (Bool true) , _) ->   raise (Interpreter.RuntimeError (
                  Interpreter.ConditionViolated (cond.cond_errors, cond.cond_expr, []), Interpreter.empty_ctx
                ))
              | new_cond_expr ->
                { p with
                  program_conds =
