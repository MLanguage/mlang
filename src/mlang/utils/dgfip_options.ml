...
let income_year =
  Arg.(value & opt int 1991 & info [ "m" ]
         ~doc:"Income year")

let application_name =
  Arg.(value & opt string "" & info [ "A" ]
         ~doc:"Application name")

let iliad_pro =
  Arg.(value & flag & info [ "R" ]
         ~doc:"Set application to both \"iliad\" and \"pro\"")

let cfir =
  Arg.(value & flag & info [ "U" ]
         ~doc:"Set application to \"cfir\"")

let batch =
  Arg.(value & opt (some int) None & info [ "b" ]
         ~doc:"Set application to \"batch\" (b0 = normal, b1 = with EBCDIC sort)")

let primitive_only =
  Arg.(value & flag & info [ "P" ]
         ~doc:"Primitive calculation only")

let extraction =
  Arg.(value & flag & info [ "X" ]
         ~doc:"Generate global extraction")

let separate_controls =
  Arg.(value & flag & info [ "S" ]
         ~doc:"Generate separate controls")

let immediate_controls =
  Arg.(value & flag & info [ "I" ]
         ~doc:"Generate immediate controls")

let overlays =
  Arg.(value & flag & info [ "o" ]
         ~doc:"Generate overlays")

let multithread =
  Arg.(value & flag & info [ "M" ]
         ~doc:"Generate multithread compatible code")

let optim_min_max =
  Arg.(value & flag & info [ "O" ]
         ~doc:"Optimize generated code (inline min_max function)")

let register =
  Arg.(value & flag & info [ "r" ]
         ~doc:"Pass TGV pointer as register in rules")

let short =
  Arg.(value & flag & info [ "s" ]
         ~doc:"Strip comments from generated output")

let output_labels =
  Arg.(value & flag & info [ "D" ]
         ~doc:"Generate labels for output variables")

let debug =
  Arg.(value & flag & info [ "g" ]
         ~doc:"Generate for test (debug)")

let nb_debug_c =
  Arg.(value & opt int 0 & info [ "k" ]
         ~doc:"Number of debug files")

let trace =
  Arg.(value & flag & info [ "t" ]
         ~doc:"Generate trace code")

let ticket =
  Arg.(value & flag & info [ "L" ]
         ~doc:"Generate calls to ticket function")

let colored_output =
  Arg.(value & flag & info [ "Z" ]
         ~doc:"Colored output in chainings")

let cross_references =
  Arg.(value & flag & info [ "x" ]
         ~doc:"Generate cross references")
...
let info =
  let doc =
    "DGFiP-specific options for Mlang."
  in
  let man =
    [
      ...
      `P
        "This option allows to specify options specific to the Mlang DGFiP backend. \
         Each option should be separated by a comma, without spaces.";
      `S Manpage.s_authors;
...
type flags = {
  ...
  xflg : bool;
      ...
      (* Other flags, not used in makefiles -h dir_var_h -i flg_ident -C flg_compact -K
         flg_optim_cte -G flg_listing (+genere_cre = FALSE) -p flag_phase -f flg_ench_init -E
...
let handler
    (income_year : int) (application_name : string)
    (iliad_pro : bool) (cfir : bool) (batch : int option)
    (primitive_only : bool) (extraction : bool)
    (separate_controls : bool) (immediate_controls : bool)
    (overlays : bool) (multithread : bool) (optim_min_max : bool)
    (register : bool) (short : bool) (output_labels : bool)
    (debug : bool) (nb_debug_c : int) (trace : bool) (ticket : bool)
    (colored_output : bool) (cross_references : bool) : flags =
  {
    nom_application = application_name; (* iliad, pro, (GP) *)
    annee_revenu = income_year;
    flg_correctif = not primitive_only;
    flg_iliad = ((application_name = "iliad" || iliad_pro) && not cfir && not (Option.is_some batch));
