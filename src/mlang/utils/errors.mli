...
(** The primary exception for all structured errors. It carries:
    ...
    - An optional thunk (continuation) to be executed after the error is
...
(** [format_structured_error ppf (msg, pos)] formats a structured error for
    **human-readable** console output. It first prints the main message,
    ...
    retrieved via [Pos.retrieve_loc_text]. If a position has an
    associated message, that message is printed just before its code snippet.

    @param ppf The OCaml [Format] module's formatter.
    @param error_data A tuple containing the main message and the list of
      associated locations. *)

val format_structured_error_gnu_format :
  Format.formatter -> string * (string option * Pos.t) list -> unit
(** [format_structured_error_gnu_format ppf (msg, pos_list)] formats an error
    according to **GNU standards** (`file:line:col: message`). For each location,
    it prints the formatted position, the main error message, and any location-specific
    message.
...
(** Raises a simple [StructuredError] with only a main message and no
    associated source code locations or continuation.

    @param msg The main error message.
    @raise StructuredError Always raises this exception with an empty list of
      locations. *)
...
(** Raises a [StructuredError] that points to a single source code location.
    ...
    @raise StructuredError Raises the exception with the provided message and a
      single-element list containing the span and its optional message. *)

val raise_multispanned_error : string -> (string option * Pos.t) list -> 'a
(** Raises a [StructuredError] that points to multiple source code
    locations, each with its own optional message.

    @param msg The main error message.
    @param spans A list of tuples, where each contains an optional
      location-specific message and a source code position ([Pos.t]).
    @raise StructuredError Raises the exception with the provided message and
      list of spans. *)
...
(** Raises a [StructuredError] for a single source location and includes a
    continuation thunk. This thunk is a function that can be executed
    by the error handler after the error message is displayed.

    @param msg The main error message.
    @param ?span_msg An optional message specific to the provided location.
    @param span The source code position ([Pos.t]) of the error.
    @param kont A function of type [unit -> unit] to be executed after error
      reporting.
    @raise StructuredError Raises the exception including the message, the span,
...
(** Prints a warning message associated with a single source code location.
    The output is formatted for human readability using [format_structured_error].
...
(** Prints a warning message associated with multiple source code locations.
    The output is formatted for human readability using
    [format_structured_error].

    @param msg The main warning message.
    @param spans A list of tuples, each containing an optional message and a
      source position. *)
