...
(** The validator performs several verifications on the consistency of the M
    ...
    this module.
    Verification requires to translate the program into an intermediate
    representation, {!Validator.program}. In this representation, rules are sorted by
    dependencies over the variables of the selected applications and domains
    (using {!Utils.TopologicalSorting}). The output type is very close to
    {!M_ir.Mir.program} and will be translated later in this type,
    by {!Mast_to_mir.translate}.
