...
type definition = DefVar of Variable.t Pos.marked | DefLiteral of literal Pos.marked

let definition_to_expression (def : definition) : expression Pos.marked =
  match def with
  | DefVar var -> Pos.same_pos_as (Var (Pos.unmark var)) var
  | DefLiteral l -> Pos.same_pos_as (Literal (Pos.unmark l)) l

type data = { numbering : ValueNumber.t NumberExpMap.t; definitions : definition ValueNumberMap.t }

let empty_data = { numbering = NumberExpMap.empty; definitions = ValueNumberMap.empty }

let rec exp_to_value_number (e : expression Pos.marked) (data : data) : ValueNumber.t * data =
  let update_data expn data =
    match NumberExpMap.find_opt expn data.numbering with
    | Some vn -> (vn, data)
    | None ->
  ...
  | LocalLet (lvar, e1, e2) ->
      ...
      let data = { data with numbering = NumberExpMap.add expne1 ne1 data.numbering } in
      exp_to_value_number e2 data

(** So normally GVN requires SSA form. What happens here with the strongly connected components ?
    Well the answer is that it does not matter. Indeed, the evaluation of the program inside SCC
    takes all the variables of a SCC as a block, updating all their values in parallel. There is no
...
let rec gvn_exp (e : expression Pos.marked) (data : data) :
    expression Pos.marked * data * ValueNumber.t =
  let expn, data = exp_to_value_number e data in
  match Pos.unmark e with
  | Literal _ -> (e, data, expn)
  | _ -> (
      match ValueNumberMap.find_opt expn data.definitions with
      | None -> (
          match Pos.unmark e with
          | Comparison (op, e1, e2) ->
