...
(** For each variable, and for each code location where it is assigned, we record the value it has
    been assigned to during an interpreter run *)

val empty_code_coverage_result : code_coverage_result

(** The code coverage is stateful, it has to be initialized before the run with [code_coverage_init]
    and you can retrieve its results after with [code_coverage_result]. *)
...
(** The accumulated coverage is the set of distinct values a particular variable assignment has
    received in the tests runs so far *)

val merge_code_coverage_single_results_with_acc :
  code_coverage_result -> code_coverage_acc -> code_coverage_acc
(** [merge_code_coverage_single_results_with_acc result acc] merges the code coverage results of a
    single run [result] with the accumulated results over the tests so far [acc] *)
