...
struct
  ...
  let empty_ctx (p : Mir.program) : ctx =
    let ctx_tgv = Array.make p.program_stats.sz_vars Undefined in
    {
      ctx_tgv;
      ctx_tmps = [];
      ctx_it = StrMap.empty;
            ...
            (fun backup (v : Mir.Var.t) ->
              ...
              let rec aux backup i =
                if i = sz then backup
                  ...
                  let value = tab.(Mir.Var.loc_int v + i) in
