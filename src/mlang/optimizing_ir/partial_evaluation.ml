...
let format_partial_expr fmt pe =
  match pe with
  | PartialLiteral l ->
      Format.fprintf fmt "PartialLiteral %a" Format_mir.format_literal l
...
let compare_for_min_dom (dom : Dominators.dom) (id1 : block_id) (id2 : block_id)
  ...
  (* the sort puts smaller items first, and we want the first item to be the
     block that is less dominating. So if id1 dominates id2, we want id2 to be
     smaller hence returning a positive value *)
...
let get_closest_dominating_def (var : Bir.variable) (ctx : partial_ev_ctx) :
  ...
  | Some previous_defs -> (
      ...
      | (def_block, Some def) :: _ ->
          (* Now we have the closest def in a block that dominates the current
             block. But something could go wrong in the case where a branch of
             an if in between the current block and the def block redefines the
             variable. So we have to check that they are no such defs *)
          if def_block = Option.get ctx.ctx_inside_block then Some def
            ...
            let exists_other_def_in_between =
              BlockMap.filter
                (fun intermediate_block ov ->
                  if
                    intermediate_block = def_block
                    || intermediate_block = curr_block
                  then false
                  else
                    Paths.check_path ctx.ctx_paths def_block intermediate_block
                    && Paths.check_path ctx.ctx_paths intermediate_block
                         curr_block
            ...
            if BlockMap.cardinal exists_other_def_in_between > 0 then
              (* if there are multiple conflicting definitions but they have the
                 same defined-ness, we keep that information *)
              let defs =
                List.map snd (BlockMap.bindings exists_other_def_in_between)
              in
              let result =
                List.fold_left
                  (fun acc od ->
...
let interpreter_ctx_from_partial_ev_ctx (ctx : partial_ev_ctx) :
    Bir_interpreter.FloatDefInterp.ctx =
  {
    Bir_interpreter.FloatDefInterp.empty_ctx with
    Bir_interpreter.FloatDefInterp.ctx_vars =
           ...
           (Bir.VariableMap.mapi
              (fun var _ ->
                ...
                | Some (SimpleVar (PartialLiteral l)) ->
                    Some
                      (Bir_interpreter.FloatDefInterp.SimpleVar
                         (Bir_interpreter.FloatDefInterp.literal_to_value l))
                | _ -> None)
              ctx.ctx_vars));
  }
...
let rec partially_evaluate_expr (ctx : partial_ev_ctx) (p : Mir.program)
  ...
  let new_e, d =
    ...
    | Binop (op, e1, e2) ->
        ...
        let new_e, d =
          match
            (Pos.unmark op, (Pos.unmark new_e1, d1), (Pos.unmark new_e2, d2))
          with
          ...
          | Mast.Add, _, (Literal (Float f), _) when f < 0. ->
              ( Binop
                  ( Pos.same_pos_as Mast.Sub op,
                    e1,
                    Pos.same_pos_as (Mir.Literal (Float (-.f))) e2 ),
                undef_cast d1 Float )
          ...
          | (Mast.Add | Mast.Sub), (_, Float), (Literal (Float 0.), _)
            when !Cli.optimize_unsafe_float ->
              (Pos.unmark new_e1, d1)
          (* substraction *)
          | Mast.Sub, (Literal (Float 0.), _), (_, Float) ->
              (Unop (Minus, new_e2), d2)
          | Mast.Sub, (e1, Float), (e2, Float)
            when e1 = e2 && !Cli.optimize_unsafe_float ->
              from_literal (Float 0.)
          | Mast.Sub, (Literal Undefined, _), (e', _) ->
              (Unop (Minus, Pos.same_pos_as e' e), d2)
          | Mast.Sub, (e', _), (Literal Undefined, _ | _, Undefined) -> (e', d1)
          (* multiplication *)
          | Mast.Mul, (Literal (Float 1.), _), (e', _) ->
              (e', undef_cast Float d2)
          | Mast.Mul, (e', _), (Literal (Float 1.), _) ->
              (e', undef_cast d1 Float)
          (* we can't optimize float multiplication by 0 here in the general
             case, because Undefined. But if we know that the other term can't
             be undefined, then we can go ! This is the case for the result of
             some functions *)
          | Mast.Mul, (Literal (Float 0.), _), (_, Float)
            when !Cli.optimize_unsafe_float ->
              from_literal (Float 0.)
          | Mast.Mul, (_, Float), (Literal (Float 0.), _)
            when !Cli.optimize_unsafe_float ->
              from_literal (Float 0.)
          (* division *)
    ...
    | Index (var, e1) ->
        ...
        let new_e, d =
          ...
          | Literal (Float f) -> (
              let idx =
                ...
                then int_of_float f
                  ...
                  let err, ctx =
                    ( Bir_interpreter.FloatDefInterp.FloatIndex
                        (Format.asprintf "%f" f, Pos.get_position e1),
                      interpreter_ctx_from_partial_ev_ctx ctx )
                  in
                  if !Bir_interpreter.exit_on_rte then
                    Bir_interpreter.FloatDefInterp.raise_runtime_as_structured
                      err ctx p
                  else
                    raise
                      (Bir_interpreter.FloatDefInterp.RuntimeError (err, ctx))
              in
          ...
          | _ ->
              if d1 = Undefined then from_literal Undefined
    ...
    | LocalVar lvar -> (
        try
          let oe, d =
            partial_to_expr (Mir.LocalVariableMap.find lvar ctx.ctx_local_vars)
          in
    ...
    | FunctionCall
        ( MaxFunc,
          [
            ...
            ( Unop
                ( Minus,
                  (FunctionCall (MaxFunc, [ (Literal (Float 0.), _); _ ]), _) ),
              _ );
          ] ) ->
    ...
    | FunctionCall
        ( MinFunc,
          [
            (Literal (Float 0.), _);
            ((FunctionCall (MinFunc, [ (Literal (Float 0.), _); _ ]), _) as
            inner_min);
          ] ) ->
        (inner_min, Float)
    | FunctionCall
        ( MaxFunc,
          [
            (Literal (Float 0.), _);
            ((FunctionCall (MaxFunc, [ (Literal (Float 0.), _); _ ]), _) as
            inner_max);
          ] ) ->
        (inner_max, Float)
    ...
    | FunctionCall (func, args) ->
        let new_args, new_ds =
          List.split
          @@ List.map (fun arg -> partially_evaluate_expr ctx p arg) args
        in
        let all_args_literal =
          List.for_all
            (fun arg ->
              match Pos.unmark arg with Mir.Literal _ -> true | _ -> false)
            new_args
        in
        ...
        let new_e, d =
          if all_args_literal then
            from_literal
              (Bir_interpreter.evaluate_expr p new_e !Cli.value_sort
                 !Cli.round_ops)
          else
            match func with
...
let partially_evaluate_stmt (stmt : stmt) (block_id : block_id)
      ...
      let new_def, new_ctx =
        ...
        | TableVar (size, def) -> (
            ...
            | IndexTable es ->
                let es' =
                  Mir.IndexMap.mapi
                    (fun _ e -> partially_evaluate_expr ctx p.mir_program e)
                    es
                in
                let new_ctx =
                  if
  ...
  | SConditional (e, b1, b2, join) -> (
      let new_e, d =
        partially_evaluate_expr ctx p.mir_program (e, Pos.no_pos)
      in
  ...
  | SVerif cond -> (
      ...
      | Some (PartialLiteral (Float _)) ->
          ...
          in
          if !Bir_interpreter.exit_on_rte then
            Bir_interpreter.FloatDefInterp.raise_runtime_as_structured err ctx
              p.mir_program
