    ...
    ctx_vars =
      Bir.VariableMap.update var
        (fun defs ->
          match defs with
          | None ->
              Some (BlockMap.singleton current_block (def, current_stmt_pos))
          | Some defs ->
              Some (BlockMap.add current_block (def, current_stmt_pos) defs))
        ctx.ctx_vars;
  }
...
let rec has_this_local_var (e : Bir.expression Pos.marked)
  ...
  | Mir.FunctionCall (_, args) ->
      List.exists (fun arg -> has_this_local_var arg l) args
  | Mir.Conditional (e1, e2, e3) ->
      has_this_local_var e1 l || has_this_local_var e2 l
...
let rec inline_in_expr (e : Bir.expression) (ctx : ctx)
                      ...
                      BlockMap.exists
                        (fun intermediate_block (_, intermediate_block_pos) ->
                          if intermediate_block = previous_x_def_block_id then
                            ...
                            Paths.check_path ctx.ctx_paths
                              previous_x_def_block_id intermediate_block
  ...
  | Mir.Comparison (op, e1, e2) ->
      let new_e1 =
        Pos.same_pos_as
          (inline_in_expr (Pos.unmark e1) ctx current_block current_pos)
          e1
      in
      let new_e2 =
        Pos.same_pos_as
          (inline_in_expr (Pos.unmark e2) ctx current_block current_pos)
          e2
      in
  ...
  | Mir.Conditional (cond, e_t, e_f) ->
      let new_cond =
        Pos.same_pos_as
          (inline_in_expr (Pos.unmark cond) ctx current_block current_pos)
          cond
      in
      let new_e_t =
        Pos.same_pos_as
          (inline_in_expr (Pos.unmark e_t) ctx current_block current_pos)
          e_t
      in
      let new_e_f =
        Pos.same_pos_as
          (inline_in_expr (Pos.unmark e_f) ctx current_block current_pos)
          e_f
      in
        ...
        if is_inlining_worthy new_e1 then
          {
            ctx with
            ctx_local_vars =
              Mir.LocalVariableMap.add l (Pos.unmark new_e1) ctx.ctx_local_vars;
          }
        else ctx
      in
      let new_e2 =
        Pos.same_pos_as
          ...
          e2
      in
      if has_this_local_var new_e2 l then Mir.LocalLet (l, new_e1, new_e2)
      else Pos.unmark new_e2
  | Mir.LocalVar l -> (
  ...
  | Mir.FunctionCall (f, args) ->
      let new_args =
        List.map
          (fun arg ->
            Pos.same_pos_as
              (inline_in_expr (Pos.unmark arg) ctx current_block current_pos)
              arg)
          args
      in
...
let inline_in_stmt (stmt : stmt) (ctx : ctx) (current_block : block_id)
  ...
  | SVerif cond ->
      let new_e =
        inline_in_expr
          (Pos.unmark cond.cond_expr)
          ctx current_block current_stmt_pos
      in
      let new_stmt =
        Pos.same_pos_as
  ...
  | SIterate (var, vcs, expr, b, b_end) ->
      let new_expr = inline_in_expr expr ctx current_block current_stmt_pos in
      let new_stmt =
          ...
          List.fold_left
            (fun (new_block, ctx, stmt_pos) stmt ->
              let new_stmt, new_ctx, new_pos =
                inline_in_stmt stmt ctx block_id stmt_pos
              in
