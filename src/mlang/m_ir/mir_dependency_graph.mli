...
(** Dependency graph for the rules of the M program. Each node corresponds to a rule, each edge to
    variables use. The edges in the graph go from input to outputs. *)

val get_used_variables_ : Mir.expression Pos.marked -> Mir.VariableDict.t -> Mir.VariableDict.t
...
val create_rules_dependency_graph : Mir.program -> Mir.rule_id Mir.VariableMap.t -> RG.t
