...
let m_operation_class : string =
  ...
  private static OptionalDouble mPresent(OptionalDouble value) {
    if(!value.isPresent()) {
      return OptionalDouble.of(0.);
    } else {
      return OptionalDouble.of(1.);
...
private static OptionalDouble mNot(OptionalDouble value) {
    if(!value.isPresent()) {
      return OptionalDouble.of(0.);
    } else if (value.getAsDouble() == 0) {
      return OptionalDouble.of(1.);
 ...
 private static OptionalDouble m_multimax(OptionalDouble bound, List<OptionalDouble> array)
{
    if (!bound.isPresent())
    {
...
let generate_java_program (program : Bir.program) (function_spec : Bir_interface.bir_function)
  ...
  Format.fprintf oc "%a%s%a%a%a%a%s" generate_header () calculateTax_method_header
