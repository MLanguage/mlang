...
let cond_DepositDefinedVariables = generate_input_condition (var_is_ "acompte")

let cond_TaxbenefitDefinedVariables = generate_input_condition (var_is_ "avfisc")
...
and translate_mpp_stmt mpp_program (m_program : Interpreter.interpretable_program) func_args
  ...
  | Mpp_ast.Partition (filter, body) ->
    let func_of_filter =
      match filter with Mpp_ast.VarIsTaxBenefit -> var_is_ "avfisc"
    in
...
and generate_partition mpp_program m_program func_args (filter : Mvg.Variable.t -> bool)
    (pos : Pos.position) (ctx : translation_ctx) : translation_ctx * Mvg.stmt list * Mvg.stmt list =
  let vars_to_move =
    Mvg.VariableMap.fold
