...
: dans (
  ...
  alors
    ...
    sinon # interdit
      ...
      PENALITE = 0;        
    finsi
    champ_evenement(R, code) reference V_MILLESIME;
...
sinon_si meme_variable(VAR, 0AV) alors
  si positif(CORR) et GLOBAL.ANNEE_DECES_CONJOINT = GLOBAL.V_MILLESIME alors
...
sinon_si S = 1 alors
  RESULTAT = (
    ...
    et non (
      meme_variable(champ_evenement(R, code), V_MILLESIME)
...
MOIS_DEP = 7;
ANNEE_DEP = GLOBAL.V_MILLESIME + 1;
R_EVT = -1;
iterer
: variable R
...
sinon_si meme_variable(VAR, 0AO) alors
  RES_SF = SF_PACSE;
sinon_si meme_variable(VAR, 0AV) alors
  si GLOBAL.ANNEE_DECES_CONJOINT = GLOBAL.V_MILLESIME alors
...
RESULTAT = (
  ...
  ou (
    vers_annee(GLOBAL.DATE_9XU) > GLOBAL.V_MILLESIME
    et vers_annee(GLOBAL.DATE_9XU) < GLOBAL.V_MILLESIME + 7
  )
);
