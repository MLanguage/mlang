...
### Add tests generating and executing C files

The `c_driver` folder contains all the necessary files for compiling a program
from C files generated by mlang. It requires to use the dgfip backend 
(`--backend dgfip_c`). To use them, create an `output` folder within the test
directory and add a symbolic link to the `c_driver` files. 

NB: Do not copy the `c_driver` files into the test directory nor in one of its
subdirectory.

## I made a change and the cram tests now fail, what should I do?

This usually happened when you break something in the code. 
Review what went wrong and, if you spot a wrong behavior, fix your code.

Failing cram tests may not always mean you broke something, it only means 
mlang's output changed, which can be what is expected.
...
This will update the `run.t` files with the new behavior. You can check the 
differences between the version without your change and with your change.
Once your solve everything, make sure to re-run the command to update the 
tests to their final version.
